...
// Harness fully encapsulates an active hcashd process, along with an embedded
// hcashwallet to provide a unified platform for creating RPC-driven integration
// tests involving hcashd. The active hcashd node will typically be run in simnet
...
// spends. The active hcashd process is fully managed by Harness, which handles
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	...
	// Start hcashwallet. This spawns a new process which will be managed
	...
	// Get a new address from the wallet to be set with hcashd's --miningaddr
	time.Sleep(5 * time.Second)
	var miningAddr hcashutil.Address
	for i := 0; i < 100; i++ {
...
// hcashd process belonging to this Harness instance. If the initial connection
