// Copyright (c) 2016 The hypercash developers
...
import (
	...
	"github.com/HcashOrg/hcashwallet/rpc/legacyrpc"
)
...
func TestMain(m *testing.M) {
	...
	ntfnHandlersNode := hcashrpcclient.NotificationHandlers{
...
func testValidateAddress(r *Harness, t *testing.T) {
	...
	for _, acct := range accounts {
		...
		// Decode address
		_, err = hcashutil.DecodeAddress(addrStr)
		if err != nil {
			t.Fatalf("Unable to decode address %s: %v", addr.String(), err)
		}

		// Try to validate an address that is not owned by wallet
		otherAddress, err := hcashutil.DecodeAddress("SsqvxBX8MZC5iiKCgBscwt69jg4u4hHhDKU")
		if err != nil {
	...
	DevAddr, err := hcashutil.DecodeAddress(devSubAddrStr)
	if err != nil {
...
func testWalletPassphrase(r *Harness, t *testing.T) {
	...
	if err != nil && err.(*hcashjson.RPCError).Code !=
		hcashjson.ErrRPCWalletPassphraseIncorrect {
		// hcashjson.ErrWalletPassphraseIncorrect.Code
	...
	// hcashjson.ErrRPCWalletUnlockNeeded
	...
	if err != nil && err.(*hcashjson.RPCError).Code !=
		hcashjson.ErrRPCWalletAlreadyUnlocked {
		t.Fatalf("WalletPassphrase failed: %v", err)
	}
...
func testGetBalance(r *Harness, t *testing.T) {
	...
	preAccountBalances := make(map[string]hcashjson.GetAccountBalanceResult)
	...
	sendAmount := hcashutil.Amount(700000000)
	...
	postAccountBalances := make(map[string]hcashjson.GetAccountBalanceResult)
...
func testListUnspent(r *Harness, t *testing.T) {
	...
	amountToSend := hcashutil.Amount(700000000)
	...
	// Get *hcashutil.Tx of send to check the inputs
	...
	for _, txIn := range rawTx.MsgTx().TxIn {
		...
		txInIDs[prevOut.String()] = hcashutil.Amount(txIn.ValueIn).ToCoin()
	}
	...
	for txinID, amt := range txInIDs {
		if _, ok := utxosBeforeSend[txinID]; !ok {
			t.Fatalf("Failed to find txid %v (%v HCASH) in list of UTXOs",
				txinID, amt)
		}
	}
...
func testSendFrom(r *Harness, t *testing.T) {
	...
	amountToSend := hcashutil.Amount(1000000)
	...
	fee := hcashutil.Amount(totalSpent - totalSent)
...
func testSendMany(r *Harness, t *testing.T) {
	...
	amountsToSend := []hcashutil.Amount{700000000, 1400000000}
	addresses := []hcashutil.Address{}
	...
	addressAmounts := make(map[hcashutil.Address]hcashutil.Amount)
	totalAmountToSend := hcashutil.Amount(0)
...
func testListTransactions(r *Harness, t *testing.T) {
	...
	if *txList1[0].TxType != hcashjson.LTTTRegular {
	...
	voutAmt := hcashutil.Amount(txOut.Value).ToCoin()
	...
	sendAmount := hcashutil.Amount(240000000)
	...
	var sendResult, recvResult hcashjson.ListTransactionsResult
	...
	rxtxResults := map[string]hcashjson.ListTransactionsResult{
...
func testGetSetRelayFee(r *Harness, t *testing.T) {
	// hcashrpcclient does not have a getwalletfee or any direct method, so we
	...
	origTxFee, err := hcashutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	// Increase fee by 50%
	newTxFeeCoin := walletInfo.TxFee * 1.5
	newTxFee, err := hcashutil.NewAmount(newTxFeeCoin)
	if err != nil {
	...
	newTxFeeActual, err := hcashutil.NewAmount(walletInfo.TxFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TxFee, err)
	}
	...
	err = wcl.SetTxFee(hcashutil.Amount(-1))
	if err == nil {
...
func testGetSetTicketFee(r *Harness, t *testing.T) {
	// hcashrpcclient does not have a getticketee or any direct method, so we
	...
	origTicketFee, err := hcashutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	newTicketFee, err := hcashutil.NewAmount(newTicketFeeCoin)
	if err != nil {
	...
	nominalTicketFee = walletInfo.TicketFee
	newTicketFeeActual, err := hcashutil.NewAmount(nominalTicketFee)
	if err != nil {
	...
	priceLimit, err := hcashutil.NewAmount(2 * mustGetStakeDiffNext(r, t))
	if err != nil {
	...
	err = wcl.SetTicketFee(hcashutil.Amount(-1))
	if err == nil {
...
func testPurchaseTickets(r *Harness, t *testing.T) {
	...
	origTicketFee, err := hcashutil.NewAmount(walletInfo.TicketFee)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
	newTicketFee, err := hcashutil.NewAmount(walletInfo.TicketFee * 1.5)
	if err != nil {
		t.Fatalf("Invalid Amount %f. %v", walletInfo.TicketFee, err)
	}
...
func mustGetStakeInfo(wcl *hcashrpcclient.Client, t *testing.T) *hcashjson.GetStakeInfoResult {
	stakeinfo, err := wcl.GetStakeInfo()
	if err != nil {
		t.Fatal("GetStakeInfo failed: ", err)
	}
...
func newBlockAt(currentHeight uint32, r *Harness,
	t *testing.T) (uint32, *hcashutil.Block, []*chainhash.Hash) {
...
func newBlockAtQuick(currentHeight uint32, r *Harness,
	...
	return block.Header.Height, hcashutil.NewBlock(block), blockHashes
}

func getBestBlock(r *Harness, t *testing.T) (uint32, *hcashutil.Block, *chainhash.Hash) {
	...
	return curBlockHeight, hcashutil.NewBlock(bestBlock), bestBlockHash
}
...
func includesTx(txHash *chainhash.Hash, block *hcashutil.Block) bool {
	if len(block.Transactions()) <= 1 {
		return false
	}
...
// includesTx checks if a block contains a transaction hash
func includesStakeTx(txHash *chainhash.Hash, block *hcashutil.Block) bool {
...
// spent minus sent.
func getWireMsgTxFee(tx *hcashutil.Tx) hcashutil.Amount {
	var totalSpent int64
	...
	return hcashutil.Amount(totalSpent - totalSent)
}

// getOutPointString uses OutPoint.String() to combine the tx hash with vout
// index from a ListUnspentResult.
func getOutPointString(utxo *hcashjson.ListUnspentResult) (string, error) {
