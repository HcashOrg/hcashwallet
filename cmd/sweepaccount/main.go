...
var (
	walletDataDirectory = hcashutil.AppDataDir("hcashwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
// Flags.
var opts = struct {
	TestNet               bool                `long:"testnet" description:"Use the test hypercash network"`
	SimNet                bool                `long:"simnet" description:"Use the simulation hypercash network"`
...
func init() {
	...
	if opts.TestNet && opts.SimNet {
		fatalf("Multiple hypercash networks may not be used simultaneously")
	}
...
func makeInputSource(outputs []hcashjson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue hcashutil.Amount
		inputs          = make([]*wire.TxIn, 0, len(outputs))
		sourceErr       error
	)
	for _, output := range outputs {
		outputAmount, err := hcashutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(hcashutil.Amount) (hcashutil.Amount, []*wire.TxIn, [][]byte, error) {
...
// function.
func makeDestinationScriptSource(rpcClient *hcashrpcclient.Client, accountName string) txauthor.ChangeSource {
...
func sweep() error {
	...
	rpcClient, err := hcashrpcclient.New(&hcashrpcclient.ConnConfig{
	...
	sourceOutputs := make(map[string][]hcashjson.ListUnspentResult)
	...
	var totalSwept hcashutil.Amount
	var numErrors int
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := hcashutil.Amount(tx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount hcashutil.Amount) bool {
	return amount >= 0 && amount <= hcashutil.MaxAmount
}

func parseOutPoint(input *hcashjson.ListUnspentResult) (wire.OutPoint, error) {
