...
//
//   https://github.com/HcashOrg/hcashwallet/blob/master/rpc/documentation/api.md
//
// Any API changes must be performed according to the steps listed here:
//
//   https://github.com/HcashOrg/hcashwallet/blob/master/rpc/documentation/serverchanges.md
package rpcserver
...
import (
	...
	pb "github.com/HcashOrg/hcashwallet/rpc/walletrpc"
...
// hcashutil.DecodeAddress, which does not perform the network check.
func decodeAddress(a string, params *chaincfg.Params) (hcashutil.Address, error) {
	addr, err := hcashutil.DecodeAddress(a)
	if err != nil {
...
// as well as establishing a RPC connection to a hcashd consensus server.
...
func (s *walletServer) NextAddress(ctx context.Context, req *pb.NextAddressRequest) (
	*pb.NextAddressResponse, error) {
	...
	pubKeyAddr, err := hcashutil.NewAddressSecpPubKey(pubKey.Serialize(), s.wallet.ChainParams())
	if err != nil {
		return nil, translateError(err)
	}
...
func (s *walletServer) ImportPrivateKey(ctx context.Context, req *pb.ImportPrivateKeyRequest) (
	*pb.ImportPrivateKeyResponse, error) {
	...
	wif, err := hcashutil.DecodeWIF(req.PrivateKeyWif)
	if err != nil {
		return nil, status.Errorf(codes.InvalidArgument,
...
func (s *walletServer) ImportScript(ctx context.Context,
	...
	p2sh, err := hcashutil.NewAddressScriptHash(req.Script, s.wallet.ChainParams())
	if err != nil {
		return nil, translateError(err)
	}
...
func (s *walletServer) FundTransaction(ctx context.Context, req *pb.FundTransactionRequest) (
	*pb.FundTransactionResponse, error) {
	...
	totalAmount, inputs, scripts, err := s.wallet.SelectInputs(hcashutil.Amount(req.TargetAmount), policy)
	...
	if req.IncludeChangeScript && totalAmount > hcashutil.Amount(req.TargetAmount) {
...
func (s *walletServer) ConstructTransaction(ctx context.Context, req *pb.ConstructTransactionRequest) (
	*pb.ConstructTransactionResponse, error) {
	...
	if req.FeePerKb != 0 {
		feePerKb = hcashutil.Amount(req.FeePerKb)
	}
...
func (s *walletServer) PurchaseTickets(ctx context.Context,
	...
	// purchase request.
	spendLimit := hcashutil.Amount(req.SpendLimit)
	if spendLimit < 0 {
		return nil, status.Errorf(codes.InvalidArgument,
	...
	txFee := hcashutil.Amount(req.TxFee)
	ticketFee := hcashutil.Amount(req.TicketFee)
...
func (t *ticketbuyerServer) StartAutoBuyer(ctx context.Context, req *pb.StartAutoBuyerRequest) (
	*pb.StartAutoBuyerResponse, error) {
	...
	var votingAddress hcashutil.Address
	...
	var poolAddress hcashutil.Address
	if req.PoolAddress != "" {
...
func (s *loaderServer) StartConsensusRpc(ctx context.Context, req *pb.StartConsensusRpcRequest) (
	*pb.StartConsensusRpcResponse, error) {
	...
	err = rpcClient.Start()
	if err != nil {
		if err == hcashrpcclient.ErrInvalidAuth {
			return nil, status.Errorf(codes.InvalidArgument,
...
func (s *messageVerificationServer) VerifyMessage(ctx context.Context, req *pb.VerifyMessageRequest) (
	*pb.VerifyMessageResponse, error) {
	...
	addr, err := hcashutil.DecodeAddress(req.Address)
	if err != nil {
		return nil, translateError(err)
	}
