...
var certificateFile = filepath.Join(hcashutil.AppDataDir("hcashwallet", false), "rpc.cert")
...
func main() {
	...
	fmt.Println("Spendable balance: ", hcashutil.Amount(balanceResponse.Spendable))
}
```
...
```bash
$ protoc -I/path/to/hcashwallet/rpc --cpp_out=. --grpc_out=. \
  --plugin=protoc-gen-grpc=$(which grpc_cpp_plugin) \
  /path/to/hcashwallet/rpc/api.proto
```
...
auto main() -> int {
    ...
    // configured to use ECDSA TLS certificates (required by hcashwallet).
    ...
    auto wallet_tls_cert_file = []{
        ...
        return pw->pw_dir + "/.hcashwallet/rpc.cert"s;
    }();
...
```PowerShell
PS> & protoc.exe -I \Path\To\hcashwallet\rpc --csharp_out=Example --grpc_out=Example `
...
namespace Example
{
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            // certificates (required by hcashwallet).
...
if (os.platform() == 'win32') {
  ...
  certPath = path.join(process.env.HOME, '.hcashwallet', 'rpc.cert');
}
...
```bash
$ protoc -I /path/to/HcashOrg/hcashwallet/rpc --python_out=. --grpc_out=. \
...
def main():
    ...
    if platform.system() == 'Windows':
        ...
        cert_file_path = os.path.join(os.environ['HOME'], '.hcashwallet', 'rpc.cert')
