...
the management of the wallet and its connection to the Hypercash network.  It has
...
dynamically start the hcashd RPC client.  This RPC client is used for wallet
syncing and publishing transactions to the Hypercash network.
...
- `string network_address`: The host/IP and optional port of the RPC server to
  ...
  default port is chosen corresponding to the default hcashd RPC port of the
  active Hypercash network.
...
- `string public_key`: The public key encoded as a string in the Hypercash encoding
  format.
...
the Hypercash network.  If the transaction spends any of the wallet's unspent
...
notifications passed from the JSON RPC of hcashd. Right now, wallet block processing
