...
// functional hypercash wallet RPC server.  This can be called to enable RPC
...
// method.  This may be a request that is handled directly by hcashwallet, or
// a chain server request that is handled by passing the request down to hcashd.
//
...
func (s *Server) handlerClosure(ctx context.Context, request *hcashjson.Request) lazyHandler {
...
// but this is only implemented here to work around an oddity with hcashjson,
...
func (s *Server) invalidAuth(req *hcashjson.Request) bool {
	cmd, err := hcashjson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*hcashjson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := hcashjson.MarshalResponse(req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := hcashjson.MarshalResponse(req.ID, nil, hcashjson.ErrRPCInvalidRequest)
		if err != nil {
			log.Errorf("Unable to marshal response to client %s: %v",
				r.RemoteAddr, err)
	...
	case "stop":
		...
		stop = true
		res = "hcashwallet stopping"
	default:
		res, jsonErr = s.handlerClosure(ctx, &req)()
	}

	// Marshal and send.
	mresp, err := hcashjson.MarshalResponse(req.ID, res, jsonErr)
	if err != nil {
		log.Errorf("Unable to marshal response to client %s: %v",
			r.RemoteAddr, err)
