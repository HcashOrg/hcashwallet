...
func StakePoolTicketFee(stakeDiff hcashutil.Amount, relayFee hcashutil.Amount,
	keyHeight int32, poolFee float64, params *chaincfg.Params) hcashutil.Amount {
	...
	initSubsidyCacheOnce.Do(func() {
		subsidyCache = blockchain.NewSubsidyCache(int64(keyHeight), params)
	})
	subsidy := blockchain.CalcStakeVoteSubsidy(subsidyCache, int64(keyHeight),
		params)
	for i := 0; i < adjs; i++ {
		subsidy *= 100
		subsidy /= 101
	...
	return hcashutil.Amount(num.Int64())
}
