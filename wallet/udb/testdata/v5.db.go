...
func setup() error {
	...
	return walletdb.Update(db, func(dbtx walletdb.ReadWriteTx) error {
		...
		wif, err := hcashutil.NewWIF(privKey, chainParams, chainec.ECTypeSecp256k1)
		if err != nil {
			return err
		}
		...
		for i := uint32(0); i < 6; i++ {
			...
			case 2, 3:
				spender, err = createUnsignedVote(&ticketRec.Hash, ticket,
					height+1, height+1, &chainhash.Hash{31: byte(height + 1)}, votebits,
...
func createUnsignedTicketPurchase(prevOut *wire.OutPoint,
	inputAmount, ticketPrice hcashutil.Amount) (*wire.MsgTx, error) {
	...
	pkScript, err = txscript.GenerateSStxAddrPush(addr,
		hcashutil.Amount(amountsCommitted[0]), ticketFeeLimits)
	if err != nil {
		return nil, err
	}
