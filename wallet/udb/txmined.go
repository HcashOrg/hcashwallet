...
type Store struct {
	...
	acctLookupFunc func(walletdb.ReadBucket, hcashutil.Address) (uint32, error)
}
...
func extractBlockHeaderVoteBits(header []byte) uint16 {
	//const voteBitsOffset = 100 //hypercash
	const voteBitsOffset = 100 + 32 //hcash
	return binary.LittleEndian.Uint16(header[voteBitsOffset:])
}

func extractBlockHeaderHeight(header []byte) int32 {
	//const voteBitsOffset = 128  //hypercash
	const heightOffset = 128 + 32 //hcash
...
func extractBlockHeaderKeyHeight(header []byte) int32{
	const keyHeightOffset = 128 + 32 + 4
	return int32(binary.LittleEndian.Uint32(header[keyHeightOffset:]))
}

func ExtractBlockHeaderKeyHeight(header []byte) int32{
	return extractBlockHeaderKeyHeight(header)
}

func ExtractBlockHeaderBits(header []byte) int32{
	return extractBlockHeaderBits(header)
}

func extractBlockHeaderBits(header []byte) int32{
	const bitsOffset = 4 + 128 + 2 + 6 + 2 + 2 + 4
	return int32(binary.LittleEndian.Uint32(header[bitsOffset:]))
}


func extractBlockHeaderUnixTime(header []byte) uint32 {
	//const timestampOffset = 136 //hypercash
	const timestampOffset = 136 + 32 + 4 //hcash
...
//type RawBlockHeader [180]byte //hypercash
type RawBlockHeader [180 + 32 + 4]byte //hcash
...
func (h *RawBlockHeader) KeyHeight() int32{
	return extractBlockHeaderKeyHeight(h[:])
}
...
func stakeValidate(ns walletdb.ReadWriteBucket, height int32) error {
	...
	for i := range blockRec.transactions {
		...
		for i, output := range txRec.MsgTx.TxOut {
			...
			minedBalance += hcashutil.Amount(output.Value)
		}
...
func (s *Store) AddCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if invalidated {
		...
		cred := credit{
			...
			block:      block.Block,
			amount:     hcashutil.Amount(rec.MsgTx.TxOut[index].Value),
			change:     change,
			spentBy:    indexedIncidence{index: ^uint32(0)},
...
func (s *Store) addCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta,
	...
	if block == nil {
		...
		v := valueUnminedCredit(hcashutil.Amount(rec.MsgTx.TxOut[index].Value),
	...
	txOutAmt := hcashutil.Amount(rec.MsgTx.TxOut[index].Value)
...
func (s *Store) addMultisigOut(ns walletdb.ReadWriteBucket, rec *TxRecord,
	...
	if block == nil {
		block = &BlockMeta{Block{*empty, 0, 0},
			rec.Received,
			0}
	}
	...
	val = valueMultisigOut(p2shScriptHash,
		...
		block.Block.Hash,
		uint32(block.Block.Height),
		hcashutil.Amount(rec.MsgTx.TxOut[index].Value),
		*empty,     // Unspent
		0xFFFFFFFF, // Unspent
		rec.Hash)
...
func approvesParent(voteBits uint16) bool {
	return hcashutil.IsFlagSet16(voteBits, hcashutil.BlockValid)
}
...
func (s *Store) outputCreditInfo(ns walletdb.ReadBucket, op wire.OutPoint,
	...
	if !mined {
		c = &Credit{
			OutPoint: op,
			BlockMeta: BlockMeta{
				Block: Block{Height: -1, KeyHeight: -1},
			},
			Amount:       amt,
			PkScript:     pkScript,
	...
	for ticketHash, _ := c.First(); ticketHash != nil; ticketHash, _ = c.Next() {
		...
		if spenderHash := existsRawUnminedInput(ns, opKey); spenderHash != nil {
			...
			if isVote, _ := stake.IsSSGen(&spender); isVote {
				voteBlock, _, _, _ := stake.SSGenBlockVotedOn(&spender)
...
CheckNext:
	...
	if k, v := latestTxRecord(it.ns, it.ck); v != nil {
		...
		it.Block = Block{Height: -1, KeyHeight: -1}
...
func (s *Store) UnspentMultisigCreditsForAddress(ns walletdb.ReadBucket,
	addr hcashutil.Address) ([]*MultisigCredit, error) {
...
func (s *Store) unspentMultisigCreditsForAddress(ns walletdb.ReadBucket,
	...
	if sha, ok := addr.(*hcashutil.AddressScriptHash); ok {
...
func (s *Store) UnspentOutputsForAmount(ns, addrmgrNs walletdb.ReadBucket,
	amt hcashutil.Amount, height int32, keyHeight int32, minConf int32, all bool,
	account uint32) ([]*Credit, error) {

	return s.unspentOutputsForAmount(ns, addrmgrNs, amt, height, keyHeight, minConf, all, account)
}
...
func (s *Store) unspentOutputsForAmount(ns, addrmgrNs walletdb.ReadBucket, needed hcashutil.Amount,
	syncHeight int32, syncKeyHeight int32, minConf int32, all bool, account uint32) ([]*Credit, error) {
	var eligible []*minimalCredit
	var toUse []*minimalCredit
	var unspent []*Credit
	found := hcashutil.Amount(0)
	...
	err := ns.NestedReadBucket(bucketUnspent).ForEach(func(k, v []byte) error {
		...
		copy(cKey[32:40], v[0:8])   // Block height
		copy(cKey[40:72], v[8:40])  // Block hash
		copy(cKey[72:76], k[32:36]) // Output index
		...
		txKeyHeight := extractRawCreditKeyHeight(cKey)
		if !confirmed(minConf, txKeyHeight, syncKeyHeight) {
			return nil
		}

		// Skip outputs that are not mature.
		if opcode == opNonstake && fetchRawCreditIsCoinbase(cVal) {
			if !confirmed(int32(s.chainParams.CoinbaseMaturity), txKeyHeight,
				syncKeyHeight) {
				return nil
			}
		}
		...
		if opcode == txscript.OP_SSTXCHANGE {
			if !confirmed(int32(s.chainParams.SStxChangeMaturity), txKeyHeight,
				syncKeyHeight) {
				return nil
			}
		}
...
type InputSource struct {
	source func(hcashutil.Amount) (hcashutil.Amount, []*wire.TxIn, [][]byte, error)
}
...
func (s *InputSource) SelectInputs(target hcashutil.Amount) (hcashutil.Amount, []*wire.TxIn, [][]byte, error) {
...
func (s *Store) MakeInputSource(ns, addrmgrNs walletdb.ReadBucket, account uint32, minConf, syncHeight int32, syncKeyHeight int32) InputSource {
	...
	var (
		currentTotal   hcashutil.Amount
		currentInputs  []*wire.TxIn
		currentScripts [][]byte
	)

	f := func(target hcashutil.Amount) (hcashutil.Amount, []*wire.TxIn, [][]byte, error) {
...
func (s *Store) balanceFullScan(ns, addrmgrNs walletdb.ReadBucket, minConf int32,
	...
	err := ns.NestedReadBucket(bucketUnspent).ForEach(func(k, v []byte) error {
		...
		cKey := make([]byte, 76)
		copy(cKey[0:32], k[0:32])   // Tx hash
		copy(cKey[32:40], v[0:8])   // Block height,keyHeight
		copy(cKey[40:72], v[8:40])  // Block hash
		copy(cKey[72:76], k[32:36]) // Output index
		...
		keyHeight := extractRawCreditKeyHeight(cKey)
		opcode := fetchRawCreditTagOpCode(cVal)
		...
		switch opcode {
		case opNonstake:
			isConfirmed := confirmed(minConf, keyHeight, syncKeyHeight)
		...
		case txscript.OP_SSTX:
			...
			votingAuthorityAmt := hcashutil.Amount(0)
			lockedByTicketsAmt := hcashutil.Amount(0)
			...
			totalInputAmount := hcashutil.Amount(0)
			...
			for _, txin := range rec.MsgTx.TxIn {
				...
				if rawUnmined != nil {
					...
					if int(txin.PreviousOutPoint.Index) < len(tx.TxOut) {
						totalInputAmount += hcashutil.Amount(tx.TxOut[txin.PreviousOutPoint.Index].Value)
					}
				} else {
...
type Balances struct {
	Account                 uint32
	ImmatureCoinbaseRewards hcashutil.Amount
	ImmatureStakeGeneration hcashutil.Amount
	LockedByTickets         hcashutil.Amount
	Spendable               hcashutil.Amount
	Total                   hcashutil.Amount
	VotingAuthority         hcashutil.Amount
	Unconfirmed             hcashutil.Amount
}
