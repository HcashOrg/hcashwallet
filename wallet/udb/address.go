...
type ManagedAddress interface {
	...
	// Address returns a hcashutil.Address for the backing address.
...
type managedAddress struct {
	manager    *Manager
	account    uint32
	address    *hcashutil.AddressPubKeyHash
	imported   bool
	internal   bool
	multisig   bool
	compressed bool
...
// Address returns the hcashutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() hcashutil.Address {
	return a.address
}
...
type scriptAddress struct {
	manager *Manager
	account uint32
	address *hcashutil.AddressScriptHash
}
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() hcashutil.Address {
	return a.address
}
...
func newScriptAddress(m *Manager, account uint32, scriptHash []byte) (*scriptAddress, error) {
	address, err := hcashutil.NewAddressScriptHashFromHash(scriptHash,
		m.chainParams)
	if err != nil {
		return nil, err
	}
