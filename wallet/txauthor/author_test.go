...
func p2pkhOutputs(amounts ...hcashutil.Amount) []*wire.TxOut {
...
func makeInputSource(unspents []*wire.TxOut) InputSource {
	...
	currentTotal := hcashutil.Amount(0)
	...
	f := func(target hcashutil.Amount) (hcashutil.Amount, []*wire.TxIn, [][]byte, error) {
		for currentTotal < target && len(unspents) != 0 {
			...
			currentTotal += hcashutil.Amount(u.Value)
...
func TestNewUnsignedTransaction(t *testing.T) {
	...
	for i, test := range tests {
		...
		if tx.ChangeIndex < 0 {
			...
			changeAmount := hcashutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
			if test.ChangeAmount == 0 {
