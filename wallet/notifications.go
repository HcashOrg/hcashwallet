...
func lookupOutputChain(dbtx walletdb.ReadTx, w *Wallet, details *udb.TxDetails,
	cred udb.CreditRecord) (account uint32, internal bool, address hcashutil.Address,
...
func makeTxSummary(dbtx walletdb.ReadTx, w *Wallet, details *udb.TxDetails) TransactionSummary {
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		...
		for _, txOut := range details.MsgTx.TxOut {
			fee -= hcashutil.Amount(txOut.Value)
		}
	}
	...
	for i := range details.MsgTx.TxOut {
		...
		output := TransactionSummaryOutput{
			...
			Account:      acct,
			Internal:     internal,
			Amount:       hcashutil.Amount(amount),
			Address:      address,
...
func totalBalances(dbtx walletdb.ReadTx, w *Wallet, m map[uint32]hcashutil.Amount) error {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
...
func flattenBalanceMap(m map[uint32]hcashutil.Amount) []AccountBalance {
...
func relevantAccounts(w *Wallet, m map[uint32]hcashutil.Amount, txs []TransactionSummary) {
	for _, tx := range txs {
...
func (s *NotificationServer) notifyUnminedTransaction(dbtx walletdb.ReadTx, details *udb.TxDetails) {
	...
	bals := make(map[uint32]hcashutil.Amount)
...
type TransactionSummaryInput struct {
	...
	PreviousAmount  hcashutil.Amount
}
