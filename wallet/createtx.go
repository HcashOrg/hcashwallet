...
func feeForSize(incr hcashutil.Amount, sz int) hcashutil.Amount {
	return hcashutil.Amount(1+sz/1000) * incr
}
...
// account.
func (w *Wallet) NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb hcashutil.Amount, account uint32, minConf int32,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		case OutputSelectionAlgorithmAll:
			...
			inputSource = func(hcashutil.Amount) (hcashutil.Amount, []*wire.TxIn, [][]byte, error) {
				total, inputs, prevScripts, err := sourceImpl.SelectInputs(hcashutil.MaxAmount)
				switch err.(type) {
...
func (s *secretSource) GetKey(addr hcashutil.Address) (chainec.PrivateKey, bool, error) {
...
func (s *secretSource) GetScript(addr hcashutil.Address) ([]byte, error) {
...
func (w *Wallet) insertIntoTxMgr(ns walletdb.ReadWriteBucket, msgTx *wire.MsgTx) (*udb.TxRecord, error) {
	...
	if err != nil {
		return nil, hcashjson.ErrInternal
	}
...
func (w *Wallet) txToOutputsInternal(outputs []*wire.TxOut, account uint32, minconf int32, chainClient *chain.RPCClient,
	...
	if atx.ChangeIndex >= 0 && account == udb.ImportedAddrAccount {
		changeAmount := hcashutil.Amount(atx.Tx.TxOut[atx.ChangeIndex].Value)
...
func (w *Wallet) txToMultisig(account uint32, amount hcashutil.Amount,
...
func (w *Wallet) txToMultisigInternal(dbtx walletdb.ReadWriteTx, account uint32,
	amount hcashutil.Amount, pubkeys []*hcashutil.AddressSecpPubKey, nRequired int8,
	minconf int32) (*CreatedTx, hcashutil.Address, []byte, error) {
	...
	txToMultisigError :=
		func(err error) (*CreatedTx, hcashutil.Address, []byte, error) {
	...
	// fee estimator.
	var feeEstForTx hcashutil.Amount
	switch {
	...
	eligible, err := w.findEligibleOutputsAmount(dbtx, account, minconf,
		amountRequired, topHeight, topKeyHeight)
	if err != nil {
		return txToMultisigError(err)
	}
	if eligible == nil {
	...
	scAddr, err := hcashutil.NewAddressScriptHash(msScript, w.chainParams)
	if err != nil {
		return txToMultisigError(err)
	}
	...
	// Request updates from hcashd for new transactions sent to this
	// script hash address.
	utilAddrs := make([]hcashutil.Address, 1)
	utilAddrs[0] = scAddr
	err = chainClient.LoadTxFilter(false, []hcashutil.Address{scAddr}, nil)
	if err != nil {
		return txToMultisigError(err)
	}
...
func (w *Wallet) compressWallet(maxNumIns int, account uint32, changeAddr hcashutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) compressWalletInternal(dbtx walletdb.ReadWriteTx, maxNumIns int, account uint32,
	changeAddr hcashutil.Address) (*chainhash.Hash, error) {
...
func makeTicket(params *chaincfg.Params, inputPool *extendedOutPoint,
	input *extendedOutPoint, addrVote hcashutil.Address, addrSubsidy hcashutil.Address,
	ticketCost int64, addrPool hcashutil.Address) (*wire.MsgTx, error) {
	mtx := wire.NewMsgTx()
	...
	zeroed := [20]byte{}
	addrZeroed, err := hcashutil.NewAddressPubKeyHash(zeroed[:], params, 0)
	if err != nil {
		return nil, err
	}
	...
	if addrPool != nil {
		pkScript, err = txscript.GenerateSStxAddrPush(addrPool,
			hcashutil.Amount(amountsCommitted[0]), limits)
		if err != nil {
	...
	pkScript, err = txscript.GenerateSStxAddrPush(addrSubsidy,
		hcashutil.Amount(amountsCommitted[userSubsidyNullIdx]), limits)
	if err != nil {
...
func (w *Wallet) purchaseTickets(req purchaseTicketRequest) ([]*chainhash.Hash, error) {
	...
	if w.addressReuse {
		...
		addrFunc = func(persistReturnedChildFunc, uint32) (hcashutil.Address, error) {
			return addr, err
		}
	}
	...
	ticketPrice, err := hcashutil.NewAmount(ticketPricesF64.NextStakeDifficulty)
	if err != nil {
		return nil, err
	}
	...
	var neededPerTicket, ticketFee hcashutil.Amount
	...
	// do so now.
	var poolFeeAmt hcashutil.Amount
	if poolAddress != nil {
		poolFeeAmt = txrules.StakePoolTicketFee(ticketPrice, ticketFee,
			tipKeyHeight, poolFees, w.ChainParams())
	...
	if req.minBalance > 0 {
		...
		estimatedFundsUsed := neededPerTicket * hcashutil.Amount(req.numTickets)
	...
	for i := 0; i < req.numTickets; i++ {
		...
		// an address.
		var addrVote, addrSubsidy hcashutil.Address
		err := walletdb.Update(w.db, func(dbtx walletdb.ReadWriteTx) error {
		...
		if eopPool != nil {
			eopPoolCredit := udb.Credit{
				...
				Amount:       hcashutil.Amount(eopPool.amt),
		...
		eopCredit := udb.Credit{
			OutPoint:     *eop.op,
			BlockMeta:    udb.BlockMeta{},
			Amount:       hcashutil.Amount(eop.amt),
...
func (w *Wallet) txToSStx(pair map[string]hcashutil.Amount,
	inputCredits []udb.Credit, inputs []hcashjson.SStxInput,
	payouts []hcashjson.SStxCommitOut, account uint32, minconf int32) (*CreatedTx, error) {
...
func (w *Wallet) txToSStxInternal(dbtx walletdb.ReadWriteTx, pair map[string]hcashutil.Amount,
	inputCredits []udb.Credit, inputs []hcashjson.SStxInput,
	payouts []hcashjson.SStxCommitOut) (tx *CreatedTx, err error) {
	...
	msgtx := wire.NewMsgTx()
	var minAmount hcashutil.Amount
	...
	for _, input := range inputs {
		...
		if err != nil {
			return nil, hcashjson.ErrDecodeHexString
		}

		if !(input.Tree == wire.TxTreeRegular ||
			input.Tree == wire.TxTreeStake) {
			return nil, hcashjson.Error{
				Code:    hcashjson.ErrInvalidParameter.Code,
		...
		totalAdded += hcashutil.Amount(input.Amt)
	}
	...
	for i := range inputs {
		...
		if payouts[i].Addr == "" {
			...
			addr, err = hcashutil.DecodeAddress(payouts[i].Addr)
			if err != nil {
				return nil, fmt.Errorf("cannot decode address: %s", err)
			}
			...
			default:
				return nil, hcashjson.ErrInvalidAddressOrKey
			}
		}
		...
		pkScript, err := txscript.GenerateSStxAddrPush(addr,
			hcashutil.Amount(payouts[i].CommitAmt), limits)
		if err != nil {
			return nil, fmt.Errorf("cannot create txout script: %s", err)
		}
		txout := wire.NewTxOut(int64(0), pkScript)
		...
		if payouts[i].ChangeAddr == "" {
			...
			a, err := hcashutil.DecodeAddress(payouts[i].ChangeAddr)
			if err != nil {
				return nil, err
			}
			// Ensure the address is one of the supported types and that
		...
		err = addSStxChange(msgtx,
			hcashutil.Amount(payouts[i].ChangeAmt),
			changeAddr)
		if err != nil {
			return nil, err
		}
...
func addSStxChange(msgtx *wire.MsgTx, change hcashutil.Amount,
	changeAddr hcashutil.Address) error {
...
func (w *Wallet) findEligibleOutputs(dbtx walletdb.ReadTx, account uint32, minconf int32,
	currentHeight int32, currentKeyHeight int32) ([]udb.Credit, error) {
	...
	for i := range unspent {
		...
		if !confirmed(minconf, output.KeyHeight, currentKeyHeight) {
			continue
		}
		...
		case class == txscript.StakeGenTy:
			...
			if !confirmed(target, output.KeyHeight, currentKeyHeight) {
				continue
			}
...
func (w *Wallet) FindEligibleOutputs(account uint32, minconf int32, currentHeight int32, currentKeyHeight int32) ([]udb.Credit, error) {
	var unspentOutputs []udb.Credit
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		var err error
		unspentOutputs, err = w.findEligibleOutputs(dbtx, account, minconf, currentHeight, currentKeyHeight)
		return err
	})
...
func (w *Wallet) findEligibleOutputsAmount(dbtx walletdb.ReadTx, account uint32, minconf int32,
	amount hcashutil.Amount, currentHeight int32, currentKeyHeight int32) ([]udb.Credit, error) {

	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)

	unspent, err := w.TxStore.UnspentOutputsForAmount(txmgrNs, addrmgrNs,
		amount, currentHeight, currentKeyHeight, minconf, false, account)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) signVoteOrRevocation(addrmgrNs walletdb.ReadBucket, ticketPurchase, tx *wire.MsgTx, isVote bool) error {
	...
	// can be performed.
	var getKey txscript.KeyClosure = func(addr hcashutil.Address) (chainec.PrivateKey, bool, error) {
	...
	var getScript txscript.ScriptClosure = func(addr hcashutil.Address) ([]byte, error) {
...
func createUnsignedVote(ticketHash *chainhash.Hash, ticketPurchase *wire.MsgTx,
	blockHeight int32, blockKeyHeight int32, blockHash *chainhash.Hash, voteBits stake.VoteBits,
	...
	subsidy := blockchain.CalcStakeVoteSubsidy(subsidyCache, int64(blockKeyHeight),
		params)
	...
	blockRefScript, _ := txscript.GenerateSSGenBlockRef(*blockHash,
		uint32(blockHeight), uint32(blockKeyHeight))
...
func createUnsignedRevocation(ticketHash *chainhash.Hash, ticketPurchase *wire.MsgTx, feePerKB hcashutil.Amount) (*wire.MsgTx, error) {
	// Parse the ticket purchase transaction to determine the required output
	// destinations for vote rewards or revocations.
	ticketPayKinds, ticketHash160s, ticketValues, _, _, _ :=
		stake.TxSStxStakeOutputInfo(ticketPurchase)
	...
	for _, output := range revocation.TxOut {
		if hcashutil.Amount(output.Value) > feeEstimate {
			amount := hcashutil.Amount(output.Value) - feeEstimate
