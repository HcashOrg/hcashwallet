...
type Wallet struct {
	...
	votingEnabled           bool
	balanceToMaintain       hcashutil.Amount
	poolAddress             hcashutil.Address
	poolFees                float64
	...
	relayFee               hcashutil.Amount
	relayFeeMu             sync.Mutex
	ticketFeeIncrementLock sync.Mutex
	ticketFeeIncrement     hcashutil.Amount
	DisallowFree           bool
	AllowHighFees          bool
	...
	addressReuse     bool
	ticketAddress    hcashutil.Address
...
func newWallet(votingEnabled bool, addressReuse bool, ticketAddress hcashutil.Address,
...
func (w *Wallet) StakeDifficulty() (hcashutil.Amount, error) {
	...
	sd, err := hcashutil.NewAmount(sdResp.NextStakeDifficulty)
	if err != nil {
		return 0, err
	}
...
func (w *Wallet) BalanceToMaintain() hcashutil.Amount {
...
func (w *Wallet) SetBalanceToMaintain(balance hcashutil.Amount) {
	w.stakeSettingsLock.Lock()
...
func (w *Wallet) RelayFee() hcashutil.Amount {
	w.relayFeeMu.Lock()
	relayFee := w.relayFee
	w.relayFeeMu.Unlock()
	return relayFee
...
// transaction) used when constructing transactions.
func (w *Wallet) SetRelayFee(relayFee hcashutil.Amount) {
	w.relayFeeMu.Lock()
	w.relayFee = relayFee
	w.relayFeeMu.Unlock()
}
...
func (w *Wallet) TicketFeeIncrement() hcashutil.Amount {
...
func (w *Wallet) SetTicketFeeIncrement(fee hcashutil.Amount) {
	w.ticketFeeIncrementLock.Lock()
...
func (w *Wallet) MainChainTipKeyHeight() (keyHeight int32) {
	...
	walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		keyHeight = w.TxStore.MainChainTipKeyHeight(txmgrNs)
		return nil
	})
	return
}
...
func (w *Wallet) loadActiveAddrs(dbtx walletdb.ReadTx, chainClient *chain.RPCClient) (uint64, error) {
	pool := sync.Pool{New: func() interface{} { return make([]hcashutil.Address, 0, 256) }}
	recycleAddrs := func(addrs []hcashutil.Address) { pool.Put(addrs[:0]) }
	getAddrs := func() []hcashutil.Address { return pool.Get().([]hcashutil.Address) }
...
func (w *Wallet) FetchHeaders(chainClient *chain.RPCClient) (count int, rescanFrom chainhash.Hash, rescanFromHeight int32,
	...
	//
	// See https://github.com/HcashOrg/hcashd/issues/427 for details.  This hack
...
func (w *Wallet) syncWithChain(chainClient *chain.RPCClient) error {
	...
	// startup.
	// TODO A proper pass through for hcashrpcclient for these cmds.
...
type (
	...
	createMultisigTxRequest struct {
		...
		pubkeys   []*hcashutil.AddressSecpPubKey
		nrequired int8
		minconf   int32
	...
	createSStxRequest struct {
		...
		pair       map[string]hcashutil.Amount
		couts      []hcashjson.SStxCommitOut
	...
	purchaseTicketRequest struct {
		minBalance  hcashutil.Amount
		spendLimit  hcashutil.Amount
		minConf     int32
		ticketAddr  hcashutil.Address
		account     uint32
		numTickets  int
		poolAddress hcashutil.Address
		poolFees    float64
		expiry      int32
		txFee       hcashutil.Amount
		ticketFee   hcashutil.Amount
...
func (w *Wallet) Consolidate(inputs int, account uint32,
	address hcashutil.Address) (*chainhash.Hash, error) {
...
func (w *Wallet) CreateMultisigTx(account uint32, amount hcashutil.Amount,
	pubkeys []*hcashutil.AddressSecpPubKey, nrequired int8,
	minconf int32) (*CreatedTx, hcashutil.Address, []byte, error) {
...
// generate a new SStx.
func (w *Wallet) CreateSStxTx(pair map[string]hcashutil.Amount,
	usedInputs []udb.Credit,
	inputs []hcashjson.SStxInput,
	couts []hcashjson.SStxCommitOut,
...
// tickets.
func (w *Wallet) PurchaseTickets(minBalance, spendLimit hcashutil.Amount,
	minConf int32, ticketAddr hcashutil.Address, account uint32,
	numTickets int, poolAddress hcashutil.Address, poolFees float64,
	expiry int32, txFee hcashutil.Amount, ticketFee hcashutil.Amount) ([]*chainhash.Hash,
	error) {
...
func (w *Wallet) PubKeyForAddress(a hcashutil.Address) (chainec.PublicKey, error) {
...
func (w *Wallet) SignMessage(msg string, addr hcashutil.Address) (sig []byte, err error) {
...
func VerifyMessage(msg string, addr hcashutil.Address, sig []byte) (bool, error) {
	...
	recoveredAddr, err := hcashutil.NewAddressSecpPubKey(serializedPK, addr.Net())
	if err != nil {
		return false, err
	}
...
func (w *Wallet) existsAddressOnChain(address hcashutil.Address) (bool, error) {
	chainClient, err := w.requireChainClient()
	if err != nil {
		return false, err
	}
...
func (w *Wallet) HaveAddress(a hcashutil.Address) (bool, error) {
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) AccountOfAddress(a hcashutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) AddressInfo(a hcashutil.Address) (udb.ManagedAddress, error) {
...
func RecvCategory(details *udb.TxDetails, syncHeight int32, syncKeyHeight int32,
	chainParams *chaincfg.Params) CreditCategory {
	if blockchain.IsCoinBaseTx(&details.MsgTx) {
		if confirmed(int32(chainParams.CoinbaseMaturity), details.Block.KeyHeight,
			syncKeyHeight) {
...
func listTransactions(tx walletdb.ReadTx, details *udb.TxDetails, addrMgr *udb.Manager,
	syncHeight int32, syncKeyHeight int32, net *chaincfg.Params) []hcashjson.ListTransactionsResult {
	...
	recvCat := RecvCategory(details, syncHeight, syncKeyHeight, net).String()

	send := len(details.Debits) != 0

	txTypeStr := hcashjson.LTTTRegular
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := hcashutil.Amount(output.Value).ToCoin()
...
func (w *Wallet) ListSinceBlock(start, end, syncHeight int32, syncKeyHeight int32) ([]hcashjson.ListTransactionsResult, error) {
	txList := []hcashjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		rangeFn := func(details []udb.TxDetails) (bool, error) {
			for _, detail := range details {
				jsonResults := listTransactions(tx, &detail,
					w.Manager, syncHeight, syncKeyHeight, w.chainParams)
...
// replies.
func (w *Wallet) ListTransactions(from, count int) ([]hcashjson.ListTransactionsResult, error) {
	txList := []hcashjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) ListAddressTransactions(pkHashes map[string]struct{}) ([]hcashjson.ListTransactionsResult, error) {
	txList := []hcashjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*hcashutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
// replies.
func (w *Wallet) ListAllTransactions() ([]hcashjson.ListTransactionsResult, error) {
	txList := []hcashjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) GetTransactions(startBlock, endBlock *BlockIdentifier, cancel <-chan struct{}) (*GetTransactionsResult, error) {
	...
	var startResp, endResp hcashrpcclient.FutureGetBlockVerboseResult
	if startBlock != nil {
...
func (w *Wallet) Accounts() (*AccountsResult, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*hcashutil.Amount)
...
func (w *Wallet) ListUnspent(minconf, maxconf int32, addresses map[string]struct{}) ([]*hcashjson.ListUnspentResult, error) {
	var results []*hcashjson.ListUnspentResult
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		for i := range unspent {
			...
			if output.FromCoinBase {
				...
				if !confirmed(target, output.KeyHeight, tipKeyHeight) {
					continue
				}
			}
			...
			case stake.TxTypeSStx:
				...
				// ticket maturity + 1??? Check this HYPERCASH TODO
				if output.Index == 0 {
					if !confirmed(int32(w.chainParams.TicketMaturity+1),
						details.KeyHeight(), tipKeyHeight) {
						continue
					}
				}
				// Change outputs.
		...
		scSwitch:
			...
			result := &hcashjson.ListUnspentResult{
...
func (w *Wallet) DumpWIFPrivateKey(addr hcashutil.Address) (string, error) {
	var privKey chainec.PrivateKey
	var done func()
	defer func() {
		if done != nil {
	...
	wif, err := hcashutil.NewWIF(privKey, w.chainParams, privKey.GetType())
	if err != nil {
		return "", err
	}
...
// wallet to disk.
func (w *Wallet) ImportPrivateKey(wif *hcashutil.WIF) (string, error) {
	chainClient, err := w.requireChainClient()
	if err != nil {
		return "", err
	}
...
func (w *Wallet) ImportScript(rs []byte) error {
	...
	return walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		err = chainClient.LoadTxFilter(false,
			[]hcashutil.Address{mscriptaddr.Address()}, nil)
		if err != nil {
			return fmt.Errorf("Failed to subscribe for address ntfns for "+
...
// a P2SH address.
func (w *Wallet) RedeemScriptCopy(addr hcashutil.Address) ([]byte, error) {
	var scriptCopy []byte
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		ns := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) StakeInfo(isLiveTicketDetails bool, chainClient *hcashrpcclient.Client) (*StakeInfoData, error) {
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for it.Next() {
			...
			if !confirmed(int32(w.chainParams.TicketMaturity)+1,
				it.Block.KeyHeight, tipKeyHeight) {
				res.Immature++
				continue
			}
			...
			if it.SpenderHash != (chainhash.Hash{}) {
				...
				case isVote(spender):
					...
					// earned by fees.
					res.TotalSubsidy += hcashutil.Amount(spender.TxIn[0].ValueIn)
...
func (w *Wallet) LockedOutpoints() []hcashjson.TransactionInput {
	locked := make([]hcashjson.TransactionInput, len(w.lockedOutpoints))
	i := 0
	for op := range w.lockedOutpoints {
		locked[i] = hcashjson.TransactionInput{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
		i++
...
func (w *Wallet) SortedActivePaymentAddresses() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr hcashutil.Address) error {
...
type AccountTotalReceivedResult struct {
	...
	AccountName      string
	TotalReceived    hcashutil.Amount
	LastConfirmation int32
}

// TotalReceivedForAccounts iterates through a wallet's transaction history,
// returning the total amount of hypercash received for all accounts.
...
// returning the total amount of hypercash received for a single wallet
// address.
func (w *Wallet) TotalReceivedForAddr(addr hcashutil.Address, minConf int32) (hcashutil.Amount, error) {
	var amount hcashutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	additionalKeysByAddress map[string]*hcashutil.WIF,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr hcashutil.Address) (
			...
			getScript := txscript.ScriptClosure(func(
				addr hcashutil.Address) ([]byte, error) {
...
func Open(db walletdb.DB, pubPass []byte, votingEnabled bool, addressReuse bool,
	...
	ticketFeeAmt, err := hcashutil.NewAmount(ticketFee)
	if err != nil {
		return nil, err
	}

	relayFeeAmt, err := hcashutil.NewAmount(relayFee)
	if err != nil {
		return nil, err
	}
