...
func (w *Wallet) handleConsensusRPCNotifications(chainClient *chain.RPCClient) {
	for n := range chainClient.Notifications() {
		...
		case chain.ClientConnected:
			log.Infof("The client has successfully connected to hcashd and " +
...
func (w *Wallet) evaluateStakePoolTicket(rec *udb.TxRecord,
	blockKeyHeight int32, poolUser hcashutil.Address) (bool, error) {
	tx := rec.MsgTx
	...
	out := hcashutil.Amount(0)
	for i := range tx.TxOut {
		out += hcashutil.Amount(tx.TxOut[i].Value)
	}
	fees := in - out
	...
	if exists {
		...
		feeNeeded := txrules.StakePoolTicketFee(hcashutil.Amount(
			tx.TxOut[0].Value), fees, blockKeyHeight, w.PoolFees(),
			w.ChainParams())
...
func (w *Wallet) processTransactionRecord(dbtx walletdb.ReadWriteTx, rec *udb.TxRecord,
	...
	height, keyHeight := int32(-1), int32(-1)
	if serializedHeader != nil {
		height = serializedHeader.Height()
		keyHeight = serializedHeader.KeyHeight()
	}
	...
	if is, _ := stake.IsSStx(&rec.MsgTx); is {
		...
		for _, addr := range addrs {
			...
			valid, errEval := w.evaluateStakePoolTicket(rec, keyHeight,
				addr)
			if valid {
		...
		if insert {
			err := w.StakeMgr.InsertSStx(stakemgrNs, hcashutil.NewTx(&rec.MsgTx))
			if err != nil {
...
func (w *Wallet) handleChainVotingNotifications(chainClient *chain.RPCClient) {
	for n := range chainClient.NotificationsVoting() {
		...
		case chain.WinningTickets:
			err = w.handleWinningTickets(n.BlockHash, int32(n.BlockHeight), int32(n.BlockKeyHeight), n.Tickets)
...
func (w *Wallet) handleWinningTickets(blockHash *chainhash.Hash,
	blockHeight int32, blockKeyHeight int32, winningTicketHashes []*chainhash.Hash) error {
	if !w.votingEnabled || blockKeyHeight + 1 < int32(w.chainParams.StakeValidationHeight)-1 {
		return nil
	}
	...
	err = walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, ticketHash := range ticketHashes {
			...
			vote, err := createUnsignedVote(ticketHash, ticketPurchase,
				blockHeight, blockKeyHeight, blockHash, voteBits, w.subsidyCache, w.chainParams)
			if err != nil {
