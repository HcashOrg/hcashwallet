...
// from them.
func (w *Wallet) MakeSecp256k1MultiSigScript(secp256k1Addrs []hcashutil.Address, nRequired int) ([]byte, error) {
	secp256k1PubKeys := make([]*hcashutil.AddressSecpPubKey, len(secp256k1Addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*hcashutil.AddressScriptHash, error) {
	var p2shAddr *hcashutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if apperrors.IsError(err, apperrors.ErrDuplicateAddress) {
				...
				p2shAddr, _ = hcashutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*hcashutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
...
func (w *Wallet) FetchP2SHMultiSigOutput(outPoint *wire.OutPoint) (*P2SHMultiSigOutput, error) {
	...
	p2shAddr, err := hcashutil.NewAddressScriptHashFromHash(
