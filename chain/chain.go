...
// RPCClient represents a persistent client connection to a hypercash RPC server
// for information regarding the current best block chain.
type RPCClient struct {
	*hcashrpcclient.Client
	connConfig        *hcashrpcclient.ConnConfig // Work around unexported field
...
func NewRPCClient(chainParams *chaincfg.Params, connect, user, pass string, certs []byte,
	...
	client := &RPCClient{
		connConfig: &hcashrpcclient.ConnConfig{
			Host:                 connect,
			Endpoint:             "ws",
			User:                 user,
			Pass:                 pass,
	...
	ntfnCallbacks := &hcashrpcclient.NotificationHandlers{
	...
	rpcClient, err := hcashrpcclient.New(client.connConfig, ntfnCallbacks)
	if err != nil {
		return nil, err
	}
...
func (c *RPCClient) Start() (err error) {
	...
	if err == nil {
		versionResult := versions["hcashdjsonrpcapi"]
		serverAPI = semver{
...
// hcashrpcclient callbacks, which isn't very Go-like and doesn't allow
...
// hypercash RPC server.  This channel must be continually read or the process
...
// downstream to the notifications queue.
func (c *RPCClient) onWinningTickets(hash *chainhash.Hash, height int64, keyHeight int64, tickets []*chainhash.Hash) {
	select {
...
out:
	for {
		...
		case <-pingChan:
			...
			// be due to hcashd processing websocket requests synchronously (see
...
func (c *RPCClient) POSTClient() (*hcashrpcclient.Client, error) {
	...
	return hcashrpcclient.New(&configCopy, nil)
}
