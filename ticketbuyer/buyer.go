...
type TicketPurchaser struct {
	...
	hcashdChainSvr     *hcashrpcclient.Client
	...
	ticketPrice      hcashutil.Amount
	stakePoolSize    uint32
	stakeLive        uint32
	stakeImmature    uint32
	stakeVoteSubsidy hcashutil.Amount
	...
	maxPriceAbsolute  hcashutil.Amount
	maxPriceRelative  float64
	maxFee            hcashutil.Amount
	minFee            hcashutil.Amount
	poolFees          float64
	maxPerBlock       int
	maxInMempool      int
	expiryDelta       int
...
func (t *TicketPurchaser) BalanceToMaintain() hcashutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetBalanceToMaintain(balanceToMaintain int64) {
	t.purchaserMtx.Lock()
	t.balanceToMaintain = hcashutil.Amount(balanceToMaintain)
	t.purchaserMtx.Unlock()
}

// MaxPriceAbsolute returns the max absolute price to purchase a ticket.
func (t *TicketPurchaser) MaxPriceAbsolute() hcashutil.Amount {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetMaxPriceAbsolute(maxPriceAbsolute int64) {
	t.purchaserMtx.Lock()
	t.maxPriceAbsolute = hcashutil.Amount(maxPriceAbsolute)
	t.purchaserMtx.Unlock()
}
...
func (t *TicketPurchaser) MaxFee() hcashutil.Amount {
	t.purchaserMtx.Lock()
	maxFee := t.maxFee
	t.purchaserMtx.Unlock()
	return maxFee
...
func (t *TicketPurchaser) SetMaxFee(maxFee int64) {
	t.purchaserMtx.Lock()
	t.maxFee = hcashutil.Amount(maxFee)
	t.purchaserMtx.Unlock()
}

// MinFee returns the min ticket fee per KB to use when purchasing tickets.
func (t *TicketPurchaser) MinFee() hcashutil.Amount {
	t.purchaserMtx.Lock()
	minFee := t.minFee
	t.purchaserMtx.Unlock()
	return minFee
...
func (t *TicketPurchaser) SetMinFee(minFee int64) {
	t.purchaserMtx.Lock()
	t.minFee = hcashutil.Amount(minFee)
	t.purchaserMtx.Unlock()
}

// TicketAddress returns the address to send ticket outputs to.
func (t *TicketPurchaser) TicketAddress() hcashutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetTicketAddress(ticketAddress hcashutil.Address) {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) PoolAddress() hcashutil.Address {
	t.purchaserMtx.Lock()
...
func (t *TicketPurchaser) SetPoolAddress(poolAddress hcashutil.Address) {
	t.purchaserMtx.Lock()
...
func NewTicketPurchaser(cfg *Config,
	...
	return &TicketPurchaser{
		...
		account:           account,
		balanceToMaintain: hcashutil.Amount(cfg.BalanceToMaintainAbsolute),
		maxFee:            hcashutil.Amount(cfg.MaxFee),
		minFee:            hcashutil.Amount(cfg.MinFee),
		maxPerBlock:       cfg.MaxPerBlock,
		maxPriceAbsolute:  hcashutil.Amount(cfg.MaxPriceAbsolute),
...
type PurchaseStats struct {
	...
	PriceAverage  hcashutil.Amount
	PriceNext     hcashutil.Amount
	PriceCurrent  hcashutil.Amount
	Purchased     int
	LeftWindow    int
	Balance       hcashutil.Amount
	TicketPrice   hcashutil.Amount
}

// Purchase is the main handler for purchasing tickets for the user.
// TODO Not make this an inlined pile of crap.
func (t *TicketPurchaser) Purchase(height int64, keyHeight int64) (*PurchaseStats, error) {

	ps := &PurchaseStats{Height: height,KeyHeight: keyHeight}
	// Check to make sure that the current height has not already been seen
	// for a reorg or a fork
	if _, exists := t.heightCheck[keyHeight]; exists {
		log.Debugf("We've already seen this keyHeight, "+
			"reorg/fork detected at keyHeight %v", keyHeight)
		return ps, nil
	}
	t.heightCheck[keyHeight] = struct{}{}

	// Initialize based on where we are in the window
	winSize := t.activeNet.StakeDiffWindowSize
	thisIdxDiffPeriod := int(keyHeight % winSize)
	nextIdxDiffPeriod := int((keyHeight + 1) % winSize)
	thisWindowPeriod := int(keyHeight / winSize)

	refreshStakeInfo := false
	if t.firstStart {
		t.firstStart = false
		log.Debugf("First run for ticket buyer")
		log.Debugf("Transaction relay fee: %v", hcashutil.Amount(t.cfg.TxFee))
	...
	t.idxDiffPeriod = int(keyHeight % winSize)
	t.windowPeriod = int(keyHeight / winSize)
	...
	if refreshStakeInfo {
		...
		subsidyCache := blockchain.NewSubsidyCache(keyHeight, t.wallet.ChainParams())
		subsidy := blockchain.CalcStakeVoteSubsidy(subsidyCache, keyHeight, t.wallet.ChainParams())
		t.stakeVoteSubsidy = hcashutil.Amount(subsidy)
	...
	if maxPerBlock == 0 {
		...
		if int(keyHeight)%maxPerBlock != 0 {
			return ps, nil
		}
		maxPerBlock = 1
	}

	avgPriceAmt, err := t.calcAverageTicketPrice(height, keyHeight)
	if err != nil {
		return ps, fmt.Errorf("Failed to calculate average ticket "+
			" price amount at height %v: %v", height, err)
	}
	if avgPriceAmt < hcashutil.Amount(t.activeNet.MinimumStakeDiff) {
		avgPriceAmt = hcashutil.Amount(t.activeNet.MinimumStakeDiff)
	}
	...
	sDiffEsts, err := t.hcashdChainSvr.EstimateStakeDiff( nil)
	if err == nil {
		ps.PriceNext, err = hcashutil.NewAmount(sDiffEsts.Expected)
		if err != nil {
			return ps, err
		}
	...
	var maxPriceAmt hcashutil.Amount
	...
	oneBlock := uint32(1)
	info, err := t.hcashdChainSvr.TicketFeeInfo(&oneBlock, &zeroUint32)
	if err != nil {
		return ps, err
	}
	...
	if !t.cfg.NoSpreadTicketPurchases && toBuyForBlock > 0 {
		...
		proportionLive := float64(0)
		if t.stakePoolSize != 0 {
		...
		log.Debugf("Your average purchase price for tickets in the pool is %.2f HCASH", yourAvgTixPrice)
		log.Debugf("Available funds of %.2f HCASH can buy %.2f tickets, %.2f tickets per block",
		...
		log.Debugf("Redeemed ticket value expected is %.2f HCASH, buys %.2f tickets, %.2f%% more",
			redeemedFunds, tixToBuyWithRedeemedFunds, tixToBuyWithRedeemedFunds/tixCanBuy*100)
		log.Debugf("Stake reward expected is %.2f HCASH, buys %.2f tickets, %.2f%% more",
	...
	// have enough moneys.
	notEnough := func(bal hcashutil.Amount, toBuy int, sd hcashutil.Amount) bool {
	...
	// Purchase tickets.
	poolFeesAmt, err := hcashutil.NewAmount(t.cfg.PoolFees)
	if err != nil {
		return ps, err
	}
