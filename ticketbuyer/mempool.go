...
func (t *TicketPurchaser) ownTicketsInMempool() (int, error) {
	...
	if t.ticketAddress != nil {
		tiHashes, err := t.hcashdChainSvr.GetRawMempool(hcashjson.GRMTickets)
		if err != nil {
			return 0, err
		}
		...
		for i := range tiHashes {
			raw, err := t.hcashdChainSvr.GetRawTransactionVerbose(tiHashes[i])
			if err != nil {
				return 0, err
			}
...
func (t *TicketPurchaser) allTicketsInMempool() (int, error) {
	tfi, err := t.hcashdChainSvr.TicketFeeInfo(&zeroUint32, &zeroUint32)
	if err != nil {
		return 0, err
	}
