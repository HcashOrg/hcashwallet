...
func (p *PurchaseManager) purchase(height int64, keyHeight int64) {
	err := p.w.Unlock(p.passphrase, nil)
	if err != nil {
		log.Errorf("Failed to purchase tickets this round: %v", err)
		return
	}
	purchaseInfo, err := p.purchaser.Purchase(height, keyHeight)
	if err != nil {
		log.Errorf("Failed to purchase tickets this round: %v", err)
		return
	}
	...
	for {
		...
		case v, ok := <-p.ntfnChan:
			...
			go func(s1, s2 chan struct{}) {
				...
				// it matches how hcashticketbuyer worked.
				if v.IsKeyBlock {
					//for h := v.NewHeight - int32(len(v.AttachedBlocks)) + 1; h <= v.NewHeight; h++ {
					//	p.purchase(int64(h))
					//}

					//for i := int32(0); i < v.KeyIncrease; i++{
						p.purchase(int64(v.NewHeight), int64(v.NewKeyHeight))
					//}
