...
func (t *TicketPurchaser) calcAverageTicketPrice(height int64, keyHeight int64) (hcashutil.Amount, error) {
	...
	// these two prices.
	var avgPricePoolAmt hcashutil.Amount
	if t.priceMode == AvgPricePoolMode || t.priceMode == AvgPriceDualMode {
		poolValue, err := t.hcashdChainSvr.GetTicketPoolValue()
		if err != nil {
			return 0, err
		}
		bestBlockH, err := t.hcashdChainSvr.GetBestBlockHash()
		if err != nil {
			return 0, err
		}
		blkHeader, err := t.hcashdChainSvr.GetBlockHeader(bestBlockH)
		if err != nil {
			return 0, err
		}
		...
		avgPricePoolAmt = poolValue / hcashutil.Amount(poolSize)
	...
	var ticketVWAP hcashutil.Amount
	if t.priceMode == AvgPriceVWAPMode || t.priceMode == AvgPriceDualMode {
		...
		startVWAPkeyHeight := uint32(0)
		if keyHeight-int64(t.cfg.AvgPriceVWAPDelta) > 0 {
			startVWAPkeyHeight = uint32(keyHeight - int64(t.cfg.AvgPriceVWAPDelta))
		}
		endVWAPKeyHeight := uint32(keyHeight)
		...
		for i := 0; i < vwapReqTries; i++ {
			ticketVWAP, err = t.hcashdChainSvr.TicketVWAP(&startVWAPkeyHeight,
				&endVWAPKeyHeight, &endVWAPHeight)
